...
<body class="font-[Poppins] pb-[83px]">
	...
	<section id="author" class="max-w-[1130px] mx-auto flex items-center flex-col gap-[30px] mt-[70px] px-4">

        <div id="title" class="flex flex-col sm:flex-row items-center gap-6 sm:gap-[30px] w-full">
        ...
        <div id="content-cards" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-[30px] w-full">
            @forelse($author->news()->latest()->get() as $news_author)
                <a href="{{ route('front.details', $news_author->slug) }}" class="card">
                    <div class="flex flex-col gap-4 p-[20px] sm:p-[26px] transition-all duration-300 ring-1 ring-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18] rounded-[20px] overflow-hidden bg-white">
                        <div class="thumbnail-container h-[200px] relative rounded-[20px] overflow-hidden">
                            <div class="badge absolute left-5 top-5 bottom-auto right-auto flex p-[8px_18px] bg-white rounded-[50px]">
                                <p class="text-xs leading-[18px] font-bold uppercase">{{ $news_author->category->name }}</p>
                            </div>
                            <img src="{{ Storage::url($news_author->thumbnail) }}" alt="thumbnail photo" class="w-full h-full object-cover" />
                        </div>
                        <div class="flex flex-col gap-[6px]">
                            <h3 class="text-lg leading-[27px] font-bold">
                                {{ substr($news_author->name, 0, 50) }}{{ strlen($news_author->name) > 50 ? '...' : '' }}
                            </h3>
                            <p class="text-sm leading-[21px] text-[#A3A6AE]">
                                {{ $news_author->created_at->diffForHumans() }}
                            </p>
                        </div>
                    </div>
                </a>
